...
function installDependencies(opts, callback) {
  ...
  fs.remove(path.join(cwd, 'node_modules'), function(error) {
    ...
    fs.remove(path.join(cwd, 'src', 'core'), function(error) {
      ...
      console.log("this is where dependencies are installed ", cwd)
      execCommand('npm run preinstall && npm install --unsafe-perm=true --loglevel error --production', { cwd }, function(error) {
