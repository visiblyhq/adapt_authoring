...
function cloneFramework (next, frameworkRepository, frameworkRevision) {
  fs.exists (path.join(serverRoot, FRAMEWORK_DIR), function (exists) {
    ...
    console.log('The Adapt Framework was not found. It will now be installed...' + frameworkRepository);
    console.log('==========================================================================')
    var child = exec('git clone --recurse-submodules' + frameworkRepository, {
      stdio: [0, 'pipe', 'pipe']
    });
    ...
    child.on('exit', function (error, stdout, stderr) {
      ...
      console.log("Clone from GitHub was successful." + frameworkRepository);
